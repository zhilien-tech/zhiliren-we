...
public final class RedisUniUtil {
	...
	*/
	...
	public static boolean exists(final IRedisDao daoIn, final String type, final String keyValue) {
		checkDao(daoIn);
		checkEmpty(type, "type不能为空!");
		checkEmpty(keyValue, "keyValue不能为空!");
	 ...
	 * @param value 值
	 */
	public static void setWithExpire(final IRedisDao daoIn, final String type, final String keyValue,
			final String value, final int seconds) {
		set(daoIn, type, keyValue, value);
		expire(daoIn, type, keyValue, seconds);
	}
