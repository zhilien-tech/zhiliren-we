...
public class SxwWwwFailProcessor extends FailProcessor {
	...
	public void process(final ActionContext ac) throws Throwable {
		...
		if (th instanceof ITimeoutException) {
			new LoginView(getLoginUrl(ac.getRequest())).render(ac.getRequest(), ac.getResponse(), th);
			return;
		}
	 ...
	 * @return 路径
	 */
	protected String getLoginUrl(HttpServletRequest httpServletRequest) {
		//TODO 先简单粗暴实现，以后再说
		StringBuilder sb = new StringBuilder();
		sb.append(kvConfig.getValue(ConfigKey.LOGIN_PAGE)).append("?m=Sso&a=login&return_url=")
				.append(HtmlUtil.urlEncode(httpServletRequest.getRequestURL().toString()));
		return sb.toString();
	}
}
