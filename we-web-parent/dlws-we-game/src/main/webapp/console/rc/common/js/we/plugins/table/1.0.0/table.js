		...
		(function($){
			$.fn.jTable = function(options){
				...
				return this.each(function(){
					...
					$('.checkboxCtrl', $table).bind('click',function(){
						var $this = $(this),
							group = $this.attr('group'),
							$group = $('input[name='+group+']',$table);
					...
					$('tbody tr',$table).each(function(){
						...
						if($this.height()>60){
							$('td',$this).each(function(){
								if($(this).find('select').size()<1){
									$(this).html('<div class="tditem">'+$(this).html()+'</div>').initUI();
								}
							});
						}
					});
					...
					//为空提示
					if($('tbody tr',$table).size()<1 && $('tbody .emptybox',$table).size()<1){
						$('tbody',$table).append('<tr><td colspan="'+$('thead th',$table).size()+'"><div class="emptybox">暂无内容</div></td></tr>');
					}
				});
				...
				return this.each(function(){
					...
					$(window).bind("resizeGrid", _resizeGrid);
				});
			};
